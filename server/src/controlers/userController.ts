...
async function obtenerRecomendacionesIA(prompt: string, datosUsuario: any): Promise<string> {
  ...
  async function calcularPorcentajesSeguridad(datosUsuario: { usuario?: any; datosMedicos: any; antecedentesObstetricos: any; embarazoActual: any; habitos: any; nutricion: any; actividadFisica: any; }) {
    const {
      usuario,
      datosMedicos,
      antecedentesObstetricos,
  ...
  function calcularPorcentajeDatosMedicos(datosMedicos: { hipertension: any; diabetes: any; hipertiroidismo: any; hipotiroidismo: any; asma: any; cancer: any; ETS: any; ansiedad: any; depresion: any; enfermedad_cardiaca: any; enfermedad_renal: any; }) {
    ...
    if (datosMedicos.hipertension) puntosRestados += 10;
    if (datosMedicos.diabetes) puntosRestados += 10;
  ...
  function calcularPorcentajeAntecedentesObstetricos(antecedentesObstetricos: { preeclampsia: any; parto_prematuro: any; hemorragias: any; perdida: any; }) {
    let puntosTotales = 100;
    let puntosRestados = 0;
  
    if (antecedentesObstetricos.preeclampsia) puntosRestados += 20;
    if (antecedentesObstetricos.parto_prematuro) puntosRestados += 25;
    if (antecedentesObstetricos.hemorragias) puntosRestados += 15;
    if (antecedentesObstetricos.perdida) puntosRestados += 10;
  
    return Math.max(0, puntosTotales - puntosRestados);
  }
  
  function calcularPorcentajeEmbarazoActual(embarazoActual: { bajo_liquido_amniotico: any; alto_liquido_amniotico: any; anomalía_fetal: any; crecimiento_disminuido: any; gestacion_multiple: any; numero_fetos: number; }) {
    let puntosTotales = 100;
    let puntosRestados = 0;
  
    if (embarazoActual.bajo_liquido_amniotico) puntosRestados += 15;
    if (embarazoActual.alto_liquido_amniotico) puntosRestados += 12;
    if (embarazoActual.anomalía_fetal) puntosRestados += 25;
    if (embarazoActual.crecimiento_disminuido) puntosRestados += 20;
    if (embarazoActual.gestacion_multiple) puntosRestados += 15;
    puntosRestados += Math.max(0, embarazoActual.numero_fetos - 1) * 10;
  
    return Math.max(0, puntosTotales - puntosRestados);
  }
  
  function calcularPorcentajeHabitos(habitos: { bebidas_alcoholicas: any; frecuencia_alcohol: string; tabaco: any; frecuencia_tabaco: string; drogas: any; hogar_libre_tabaco: any; }) {
    let puntosTotales = 100;
    let puntosRestados = 0;
  
    if (habitos.bebidas_alcoholicas) {
      puntosRestados += 30;
      if (habitos.frecuencia_alcohol === 'Diariamente') puntosRestados += 20;
    }
    if (habitos.tabaco) {
      puntosRestados += 25;
      if (habitos.frecuencia_tabaco === 'Diariamente') puntosRestados += 15;
    }
    if (habitos.drogas) puntosRestados += 40;
    if (!habitos.hogar_libre_tabaco) puntosRestados += 10;
  
    return Math.max(0, puntosTotales - puntosRestados);
  }
  
  function calcularPorcentajeNutricion(nutricion: { dieta: number; frutas: number; verduras: number; comida_rapida: number; numero_vasos: number; }) {
    let puntosTotales = 100;
    let puntosRestados = 0;
  
    if (nutricion.dieta === 2 || nutricion.dieta === 3) puntosRestados += 5; // Dieta vegetariana/vegana
    if (nutricion.frutas < 2) puntosRestados += 10;
    if (nutricion.verduras < 2) puntosRestados += 10;
    if (nutricion.comida_rapida > 2) puntosRestados += 15;
    if (nutricion.numero_vasos < 6) puntosRestados += 10;
  
    return Math.max(0, puntosTotales - puntosRestados);
  }
  
  function calcularPorcentajeActividadFisica(actividadFisica: { actividad_fisica: any; frecuencia_actividad: string; tiempo_actividad: string; }) {
    let puntosTotales = 100;
    let puntosRestados = 0;
    ...
    if (!actividadFisica.actividad_fisica) {
      ...
      if (actividadFisica.frecuencia_actividad === 'Raramente') puntosRestados += 30;
      if (actividadFisica.tiempo_actividad === 'Menos de 15 minutos') puntosRestados += 20;
    }
  
    return Math.max(0, puntosTotales - puntosRestados);
  }
  
  function getNivelRiesgo(porcentaje: number) {
    if (porcentaje >= 90) return "Bajo riesgo";
    if (porcentaje >= 75) return "Riesgo moderado bajo";
    if (porcentaje >= 60) return "Riesgo moderado";
    if (porcentaje >= 45) return "Riesgo moderado alto";
    return "Alto riesgo";
  }